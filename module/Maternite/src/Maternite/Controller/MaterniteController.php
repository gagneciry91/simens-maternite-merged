...
use Maternite\Form\ProtocoleOperatoireForm;
use Maternite\View\Helpers\CompteRenduOperatoirePdf;
use Maternite\View\Helpers\CompteRenduOperatoirePage2Pdf;

class MaterniteController extends AbstractActionController
{
...
//var_dump($id_cons);exit();
    ...
    {
        $this->layout()->setTemplate('layout/consultationm');
        // var_dump( uniqid(md5(rand()), true) ); //nombre alï¿½atoire
        $user = $this->layout()->user;
        $IdDuService = $user ['IdService'];
    ...
    public function visualisationHospitalisationAction()
    {
        ...
        // Pour les infos sur le transfert du patient hospitalisï¿½
    ...
    public function raffraichirListeSoinsPrescritTerminer($id_hosp)
    {
        ...
        foreach ($liste_soins as $cle => $Liste) {
            // Rï¿½cupï¿½ration de l'heure suivante pour l'application du soin par l'infirmier
    ...
    public function imagesExamensMorphologiquesAction()
    {
        ...
        if ($ajout == 1) {
            ...
            if ($idExamen == 8) {
                $nomImage = "radio_" . $nomImage;
            }
            if ($idExamen == 9) {
                $nomImage = "echographie_" . $nomImage;
            }
            if ($idExamen == 10) {
                $nomImage = "irm_" . $nomImage;
            }
            if ($idExamen == 11) {
                $nomImage = "scanner_" . $nomImage;
            }
            if ($idExamen == 12) {
                $nomImage = "fibroscopie_" . $nomImage;
            }
    ...
    public function raffraichirListeSoinsPrescrit($id_hosp)
    {
        ...
        foreach ($liste_soins as $cle => $Liste) {
            ...
            if ($heureSuivante || $heurePrecedente) {
                ...
                } else if ($heurePrecedenteH == $heureActuelleH) {
                    $heureActuelleM = $today->format('i');
    ...
    public function getLesHeuresDuSoin($id_sh, $date)
    {
        ...
        // S'il n'y a plus une heure suivante c'est a dire toutes les heures sont passï¿½es
    ...
    public function libererPatientAction()
    {
        ...
        /**
         * La libï¿½ration du lit ici maintenant effectuï¿½ par le major du service hospitalisation
         * * La libï¿½ration du lit ici maintenant effectuï¿½ par le major du service hospitalisation
         *
    ...
    public function lecteurVideoAction($ListeDesVideos)
    {
        ...
        foreach ($ListeDesVideos as $Liste) {
            ...
            } else if ($Liste ['format'] == "video/webm") {
                $html .= '<script>
        		 tab[' . $i++ . '] = {
	                     "title":"' . $Liste ['titre'] . '<span class=\'supprimerVideoIns' . $i . '\' >  </span>",
    ...
    public function vueExamenJourAction()
    {
        ...
        $formSoin->populateValues(array_merge($data, $Constantes, $DonneesMotifs, $bandelettes, $DonneesExamenPhysique, $motif_admission));
    ...
    public function vuePatientOpereBlocAction()
    {
        ...
        // Informations sur le protocole opï¿½ratoire
    ...
    public function modifierProtocoleOperatoireAction()
    {
        ...
        return $this->redirect()->toRoute('consultation', array(
            'action' => 'liste-protocole-operatoire'
        ));
    }
    ...
    public function afficherMp3ProtocoleAction()
    {
        ...
        // $ListeDesSons = $this->getConsultationTable ()->getMp3($id_cons, $type);
        $html = "";
        $html = $this->lecteurMp3ProtocoleAction(null);
    ...
    public function lecteurMp3ProtocoleAction($ListeDesSons)
    {
        ...
        // tab['.$i++.'] = {
        // "title":"'. $Liste['titre'] .'<span class=\"supprimerSon'.$i.'\" > </span>",
        // "mp3":"'. $this->cheminBaseUrl().'public/audios/'. $Liste['nom'] .'",
        // };
        ...
        $html .= '<script>
		        ...
		        swfPath: "' . $this->cheminBaseUrl() . 'public/js/plugins/jPlayer-2.9.2/dist/jplayer",
		        supplied: "mp3",
		        wmode: "window",
		        useStateClassSkin: true,
		        autoBlur: false,
